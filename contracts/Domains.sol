...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory _tld) payable ERC721 ("Beedu Name Service", "BNS") {
        tld = _tld;
    ...
    function price(string calldata name) public pure returns(uint) {
        ...
        if (len == 3) {
	        ...
	        return 3 * 10**17; // To charge smaller amounts, reduce the decimals
        } else {
	        return 1 * 10**17;
        }
    }
      ...
      bytes(
        string(
          abi.encodePacked(
            '{"name": "',
            _name,
            '", "description": "A domain on the Beedu Name Service", "image": "data:image/svg+xml;base64,',
